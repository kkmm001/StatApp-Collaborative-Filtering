...
vect.MoviesInCommon = as.matrix(intersect(mat.MoviesOfuserID1$movieID, mat.MoviesOfuserID2$movieID))
  ...
  for (userIND1 in 1:(nb.Users-1)){
    ...
    for(userIND2 in (userID1+1):nb.Users){
      ...
      mat.com[userIND1, userIND2] = length(intersect(mat.MoviesOfuserID1$movieID, mat.MoviesOfuserID2$movieID))

      hist(mat.com, breaks = 100, xlim = c(0,100))
      table(mat.com)
    }
  }
```
...
```{r, echo = FALSE}
mat.sim_pearson = as.matrix(read.table(file = paste0("./Results/", repository, "/mat.sim_pearson.tsv"), header=T, sep='\t'))
...
```{r, echo = FALSE}
mat.sim_nrmse = as.matrix(read.table(file = paste0("./Results/", repository, "/mat.sim_nrmse.tsv"), header=T, sep='\t'))
...
```{r, echo = FALSE}
mat.sim_nmae = as.matrix(read.table(file = paste0("./Results/", repository, "/mat.sim_nmae.tsv"), header=T, sep='\t'))
