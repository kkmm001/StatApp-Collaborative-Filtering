---
output: pdf_document
title: "Filtrage collaboratif - Analyse de ml-1m"
...
```{r,echo = FALSE}
data.Ratings = read.table(file = "../Data/ml-1m/data.Ratings.tsv", header=T, sep='\t')
data.Movies = read.table(file = "../Data/ml-1m/data.Movies.tsv", header=T, sep='\t')
data.Users = read.table(file = "../Data/ml-1m/data.Users.tsv", colClasses = c("integer", "factor", "integer", "factor", "factor"), header=T, sep='\t')
...
%## Nettoyage de la base data.Movies (à mettre dans clean)
```{r, echo = FALSE}
#vect.Movies_Ratings = sort(unique(data.Ratings$movieID))
#vect.Movies_Movies = sort(unique(data.Movies$movieID))
#vect.MoviesToDelete = vect.Movies_Movies[!(vect.Movies_Movies %in% vect.Movies_Ratings)]
#data.Movies = data.Movies[!(data.Movies$movieID %in% vect.MoviesToDelete),]
#write.table(data.Movies, "../Data/ml-1m/Data.Movies2.tsv", row.names = FALSE, sep='\t')
```

```{r,echo = FALSE}
## Taux de complétion de la matrice Y
taux_completion = nb.Ratings/(nb.Users * nb.Movies)
...
La répartition des âges est étrange : il y a des données incorrectes (1 an) et par classes (20-25 ans sur-représentés et 26-30 ans sous-représentés)
...
Remarque : le code postal n'est pas du tout pertinent (5111 other)

## Sur les films

```{r, echo = FALSE}
## Repartition par genres de films
vect.nb.MoviesPerGenre = as.matrix(apply(data.Movies[3:21],2, sum, na.rm = TRUE))
...
### Utilisateurs n'ayant vote moins que 20 fois
