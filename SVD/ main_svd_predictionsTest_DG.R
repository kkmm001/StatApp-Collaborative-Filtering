...
matrix.training = transform.data.rating(  train.Ratings, vector.userID, vector.movieID)
 ...
 pred = restablish.data.rating.col(matrix.prevision, vector.userID, vector.movieID)
    ...
    result_RMSE[INDL,testID] = rmse(test.Ratings$rating, pred)
  }
}

result=as.data.frame(result_RMSE)
rownames(result)=seq_lambda
write.csv2(result,paste0("./Results/Results.RMSE.SVD.DG. iteration.times ", iteration.times ,".csv"),col.names = NA)
...
for(INDL in 1:length(seq_lambda)){
 ...
 pred = restablish.data.rating.col(matrix.prevision, vector.userID, vector.movieID)
    write.csv2(pred, paste0("svd.DG_pred_sur_base_vierge.iteration.times.”, iteration.times, “.L.”,L,".csv"), col.names = NA)
    result_RMSE2[INDL,1] = rmse(test.Ratings$rating, pred)
    }
Result2=as.data.frame(result_RMSE2)
rownames(result2)=seq_lambda
write.csv2(result,paste0("./Results/Results.RMSE.SVD.DG.pred_sur_base_vierge.Iteration_times", iteration.times ,".csv"),col.names = NA)
